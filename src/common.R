...
# DRE: my additions to the common library
# returns string w/o leading or trailing whitespace
trim <- function (x) gsub("^\\s+|\\s+$", "", x)


# Returns a default value (NULL if none is provide) if the value is blank or a numeric if the value parses as numeric.
# Otherwise it stops execution with an error.
require.numeric.or.default <- function(value, paramName, default=NULL) {
    if (is.null(value)) { return(default) }
    trimmedValue = trim(value)
    if (trimmedValue == "") { return(default) }
...
isGct <- function(file.name) {
    return (regexpr("\\.gct$", tolower(file.name))[[1]] != -1)
}

isRes <- function(file.name) {
    return (regexpr("\\.res$", tolower(file.name))[[1]] != -1)
}

isGctOrRes <- function(file.name) {
    return (regexpr("\\.gct$|\\.res$", tolower(file.name))[[1]] != -1)
}

isCsv <- function(file.name) {
    return (regexpr("\\.csv$", tolower(file.name))[[1]] != -1)
}
